...
/** ログ出力を自動化するAspectクラス @Loggableアノテーションが付けられたメソッドの実行前後に自動でログを出力 */
@Aspect
@Component
public class LoggingAspect {
  ...
  public Object logExecutionTime(ProceedingJoinPoint joinPoint, Loggable loggable)
    ...
    try {
      ...
      String argsInfo =
          loggable.includeArgs() && args.length > 0
      ...
      // メソッド実行終了ログ
      String resultInfo =
          loggable.includeResult() && result != null
      ...
      LoggingUtils.logError(
          targetLogger, String.format("メソッド実行エラー: %s.%s", className, methodName), e);
      throw e;
    }
  }
  ...
  public Object monitorPerformance(
      ProceedingJoinPoint joinPoint, PerformanceMonitoring performanceMonitoring) throws Throwable {
    MethodSignature signature = (MethodSignature) joinPoint.getSignature();
    String className = signature.getDeclaringTypeName();
    String methodName = signature.getName();
    String operation =
        !performanceMonitoring.operation().isEmpty()
    ...
    try {
      ...
      if (executionTime > performanceMonitoring.threshold()) {
        targetLogger.warn(
            "パフォーマンス警告: {} の実行時間が閾値を超えました。実行時間: {}ms, 閾値: {}ms",
            operation,
            executionTime,
            performanceMonitoring.threshold());
      }

      return result;

    } catch (Exception e) {
      LoggingUtils.logError(targetLogger, String.format("パフォーマンス監視対象メソッドでエラー: %s", operation), e);
      throw e;
    }
  }
  ...
  private Object[] sanitizeArgs(Object[] args) {
    return Arrays.stream(args).map(this::sanitizeForLog).toArray();
  }
