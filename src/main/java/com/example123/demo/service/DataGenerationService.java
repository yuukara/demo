...
/** ランダムな従業員データを生成するサービスクラス テストデータ作成やデモンストレーション用途に使用されます */
@Service
@Validated
public class DataGenerationService {
  ...
  private static final String[] POSITIONS = {"部長", "次長", "課長", "係長", "主任", "担当", "アシスタント"};

  private static final String[] EMPLOYMENT_STATUSES = {"正社員", "契約社員", "パートタイマー", "アルバイト"};

  private final Random random = new Random();

  /**
   * 指定された件数のランダムな従業員データを生成します。 日本の名前や一般的な部署名などを使用し、よりリアルなデータを生成します。
  ...
  public List<Employee> createRandomEmployees(
      @Min(value = 1, message = "生成件数は1以上である必要があります") int count) {
  ...
  public Employee createEmployeeWithId(String id, LocalDateTime now) {
    ...
    employee.setPhone_number(
        String.format("03-%04d-%04d", random.nextInt(10000), random.nextInt(10000)));

    employee.setEmail("employee" + id + "@example.com");

    // 生年月日は22-60歳の範囲
    employee.setBirth_date(
        LocalDate.now().minusYears(22 + random.nextInt(38)).minusDays(random.nextInt(365)));
  ...
  public List<Employee> createDummyEmployees(
      @Min(value = 1, message = "生成件数は1以上である必要があります") int count) {
    List<Employee> employees = new ArrayList<>();
    LocalDateTime now = LocalDateTime.now();
    ...
    for (int i = 0; i < count; i++) {
      ...
      employee.setPhone_number(
          "03-" + String.format("%04d", i % 10000) + "-" + String.format("%04d", (i + 1) % 10000));
      employee.setEmail("employee" + id + "@example.com");
  ...
  /**
   * UPSERT処理用の基礎データを準備します。 更新対象となるデータ（E000000-E009999）を事前にテーブルに投入するためのデータを生成します。
   *
